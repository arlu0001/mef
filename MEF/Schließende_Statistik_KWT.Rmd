...
kruskal.test(data_time_config_long$`Time in ms`,data_time_config_long$Reaction)
```
...
kruskal.test(data_time_config_long$`Time in ms`,data_time_config_long$Reaction)
```
### P-Wert = 2.2e-16 somit kleiner als 0.05 und die Nullhypothese

## Post-hoc-Analyse:
```{r}
# pairwise.wilcox.test(data_time_config_long$`Time in ms`,data_time_config_long$Reaction, p.adjust="bonferroni")
