...
```{r}
fig <- plot_ly(type = "box")
fig <- fig %>% add_trace(y = data_ishiharaclean$`Richtig`)
#fig <- fig %>% add_trace(y = data_ishiharafalse$`Richtig`, name="mit Farbsehschwäche")
fig <- fig %>% layout(
    xaxis = list(showticklabels = FALSE),
    yaxis = list(title= "Anzahl richtig beantworteter Aufgaben"),
    showlegend = FALSE
    #title = "Verteilung richtiger Aufgaben"
  )
...
```{r}
fig <- plot_ly(type = "box")
fig <- fig %>% add_trace(y = correct_long_1$`Time in ms`, name="Config 1")
fig <- fig %>% add_trace(y = correct_long_3$`Time in ms`, name="Config 3")
fig <- fig %>% add_trace(y = correct_long_4$`Time in ms`, name="Config 4")
fig <- fig %>% add_trace(y = correct_long_5$`Time in ms`, name="Config 5")
fig <- fig %>% add_trace(y = correct_long_6$`Time in ms`, name="Config 6")
fig <- fig %>% add_trace(y = correct_long_7$`Time in ms`, name="Config 7")
fig <- fig %>% add_trace(y = correct_long_8$`Time in ms`, name="Config 8")
